...
public String write(RoomDto roomDto,MultipartFile file,HttpSession session,HttpServletRequest request) throws Exception {    // 입력한 내용을 받아와야하니깐 CommentDto dto 해줘야한다.
   ...
   @ResponseBody public ResponseEntity<String> modify(@PathVariable Integer bno,@RequestBody RoomDto ndto, HttpSession session) throws Exception {    // 입력한 내용을 받아와야하니깐 CommentDto dto 해줘야한다.
        String writer = (String)session.getAttribute("id");
